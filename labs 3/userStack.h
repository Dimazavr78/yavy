...
template<class value_type> void Container<value_type>::push(const value_type
	dat)
{
	std::cout << "Parent class\n";
}
...
template<class value_type> Container<value_type>&
Container<value_type>::operator=(Container<value_type>& temp)
{
	std::cout << "Parent class\n";
	return *this;
}
...
template<class value_type> void Vector<value_type>::push(const value_type
	dataPush)
{
	// Проверка на заполненность стэка
	if (Container<value_type>::_size_ != _MAX_SIZE_ARR)
	{
		Container<value_type>::arr[Container<value_type>::_size_++] =
			dataPush;
	}
	else
	{
		std::cout << "Стэк заполнен. Из стека был извлечён элемент" <<
			this->top() << "и вместо него был добавлен элемент " << dataPush << "\n";
		Container<value_type>::arr[Container<value_type>::_size_ - 1] =
			dataPush;
		system("Pause");
	}
}
...
template<class value_type> Vector<value_type>&
Vector<value_type>::operator=(Vector<value_type>& temp)
{
	// Проверка на эквивалентность двух переменных
	if (this == &temp)
		return *this;
	// Создание вспомогательного типа данных
	Vector<value_type> tempOne;
	// Очистка памяти массива + заполнение новыми данными
	delete[] this->arr;
	this->arr = temp.arr;
	return *this;
}
...
Vector<value_type>::operator=(Vector<value_type>&& temp)
{
	...
	// Очистка данных в правом операнде
	temp.arr = nullptr;
	temp._size_ = 0;
	return *this;
}
...
template<class value_type> void List<value_type>::push(const value_type
	dataPush)
{
	if (Container<value_type>::_size_ == _MAX_SIZE_ARR)
	{
		std::cout << "Невозможно добавление новых данных. Стэк заполнен!\n";
	}
...
template<class value_type> List<value_type>&
List<value_type>::operator=(List<value_type>& temp)
{
	// Проверка на эквивалентность двух переменных
	if (this == &temp)
		return *this;
	...
	// Очищение данных левого операнда при присваивании
	while (Container<value_type>::_size_-- && Container<value_type>::tail)
	{
...
List<value_type>::operator=(List<value_type>&& temp)
{
	...
	if (this == &temp)
		return *this;
	// Перемещение данных левого операнда в правый
...
#pragma region Stack
// Шаблонный класс стэка
template <class value_type, template<typename> class Container = Vector>
class Stack
{
public:
	Stack() = default;
